...
export default function FormIscrizioneImpresa() {
  ...
  const validate = () => {
    ...
    if (form.partita_iva && !PIVA_REGEX.test(form.partita_iva.replace(/\s/g, "")))
      e.partita_iva = "P.IVA non valida (11 cifre)";
    ...
    try {
      ...
      if (form.newsletter) {
        await supabase.from("newsletter").upsert(
          { email: form.email.trim().toLowerCase(), nome: form.nome.trim(), attivo: true },
    ...
    return (
      ...
      >
        ...
        <p className="font-sans text-gray-600 text-base leading-relaxed max-w-lg mx-auto">
          Abbiamo registrato la tua richiesta. Ti contatteremo entro 24 ore per confermare
          i dettagli di iscrizione e il percorso di pagamento.
        </p>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormField label="Partita IVA" error={errors.partita_iva}>
            <input type="text" name="partita_iva" value={form.partita_iva} onChange={handleChange}
              maxLength={11} className={inputClass} style={{ ...inputStyle(!!errors.partita_iva), fontFamily: "monospace" }}
          ...
          <FormField label="Codice Fiscale (azienda o persona fisica)">
            <input type="text" name="codice_fiscale_azienda" value={form.codice_fiscale_azienda} onChange={handleChange}
              maxLength={16} className={inputClass} style={{ ...inputStyle(false), fontFamily: "monospace" }}
        ...
        <div className="mt-4">
          <FormField label="Note aggiuntive">
            <textarea name="note" value={form.note} onChange={handleChange} rows={3}
              ...
              placeholder="Eventuali informazioni utili per gestire la tua iscrizione…" />
          </FormField>
        </div>
      </section>
        ...
        <div className="flex flex-col gap-4">
          <div>
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="font-sans text-sm text-gray-700 leading-relaxed">
                ...
                per la gestione della mia iscrizione.{" "}
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" name="newsletter" checked={form.newsletter} onChange={handleChange}
              className="mt-1 w-4 h-4 rounded shrink-0" style={{ accentColor: "#8b0000" }} />
            <span className="font-sans text-sm text-gray-700 leading-relaxed">
              Desidero ricevere aggiornamenti sui percorsi formativi e le opportunità di Formazione Impresa
              (opzionale, disiscrizione sempre possibile).
            </span>
          </label>
        </div>
      </section>
