...
const INITIAL = {
  ...
  azienda: "", messaggio: "", consenso_gdpr: false,
};
...
export default function FormContattoImpresa() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Log interazione
      await logInterazione(contatto_id, "impresa_contatto", "contatto", "Formazione Impresa", {
        azienda: form.azienda.trim() || null,
        messaggio: form.messaggio.trim() || null,
      });
  ...
  if (success) {
    return (
      <div ref={topRef} className="rounded-2xl p-8 text-center" style={{ backgroundColor: "#fff5f5", border: "2px solid #8b0000" }}>
        ...
        <h3 className="font-display font-bold text-2xl mb-3" style={{ color: "#8b0000" }}>Richiesta inviata!</h3>
    ...
    <form ref={topRef} onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
      ...
      <FormField label="Come possiamo aiutarti?">
        <textarea name="messaggio" value={form.messaggio} onChange={handleChange} rows={4}
          className={inputClass} style={inputStyle(false)} placeholder="Descrivi brevemente le tue esigenze formative…" />
      </FormField>
